...
export default function App() {
    return (
        <Canvas orthographic camera={{ position: [6, -5, 10], zoom: 20 }}>
        <color attach="background" args={['#2C3333']} />
        <ambientLight />
...
function Scene({ ...props }) {
    ...
    return (
        <>
            <group {...props}>
            <PivotControls scale={100} activeAxes={[true, true, false]} offset={[0, 0, 100]}>
...
function Shape({ name, float = 300, color, config, ...props }: any) {
